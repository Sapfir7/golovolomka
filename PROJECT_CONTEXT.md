...
## 3D scene and assets
- **Файл сцены:** `miniapp-3d/public/1res_08042026.glb` → после сборки `miniapp-3d-dist/1res_08042026.glb`, URL **`/miniapp-3d/1res_08042026.glb`**. `useGLTF` + `scene.clone(true)`; свет и материалы только из GLB.
- **Имена объектов (Blender → GLB):** слоты **`slot00`…** (пустышки); экран **`Screen`**; камеры **`Camera`** (полка), **`Camera_to_00`…`Camera_to_03`** (к экрану), **`Camera_out_04` / `Camera_out_05`** (назад); траектория шара **`pos_00` → `pos_01` → `pos_02` → `finish`** (скрыты в рендере); луч **`Cone`**; константы в `miniapp-3d/src/constants/gltfSceneNodes.ts`.
...
- Превью: `previewTextureCache.ts` (canvas + EXIF), перед `SHELF` — `awaitPreviewLoads`; на экране **`Screen`** — шейдер `screenProjectionMaterial.ts`. Шары на полке сейчас без фото (только стекло/цвет).
